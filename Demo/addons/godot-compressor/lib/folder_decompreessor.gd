...
func decompress(to : String, compression_mode := 0) -> int:
	...
	buffer = _decompress_buffer(buffer, compression_mode)
	
	var tree : Dictionary = bytes2var(buffer)
	_decompress_tree(tree, compression_mode)
...
func _decompress_buffer(buffer : PoolByteArray, compression_mode := 0) -> PoolByteArray:
	if buffer.empty():
		return buffer
...
func _decompress_tree(tree, compression_mode := 0) -> void:
	var keys
	if tree is Array:
		keys = tree.size()
	...
	for i in keys:
		if tree[i] is PoolByteArray and not tree[i].empty():
			tree[i] = _decompress_buffer(tree[i], compression_mode)
		elif tree[i] is Array or tree[i] is Dictionary:
			_decompress_tree(tree[i], compression_mode)


func _store_tree(tree : Dictionary, to : String):
	if not to.ends_with("/"):
		to += "/"
